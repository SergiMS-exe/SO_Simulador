...
void OperatingSystem_FillProcessesTimesQueue();
void OperatingSystem_PrintProcessTimes(int);
...
void OperatingSystem_Initialize(int daemonsIndex) {
	...
	OperatingSystem_FillProcessesTimesQueue(); //V2 Ej 1 simulacro
...
void OperatingSystem_TerminateProcess() {
	...
	OperatingSystem_PrintProcessTimes(pid);
...
void OperatingSystem_MoveToTheBLOCKEDState(int PID) {
	...
	if (Heap_add(executingProcessID, sleepingProcessesQueue, QUEUE_WAKEUP, &numberOfSleepingProcesses, PROCESSTABLEMAXSIZE)>=0){
		if (notExecutedYet[PID]==0)
			notExecutedYet[PID]=1;
...
void OperatingSystem_UpdateProcessesTimes() {
	int responseTime, returnTime, waitingTime, i;
	
	returnTime = processesTimesQueue[executingProcessID][RETURNTIME];
	processesTimesQueue[executingProcessID][RETURNTIME]=returnTime+1;
	returnTime=0;


	for (i=0; i<PROCESSTABLEMAXSIZE;i++){
		if (i!=executingProcessID){

			if (processTable[i].state!=EXIT){ //Sumamos tiempo de retorno
				returnTime=processesTimesQueue[i][RETURNTIME];
				processesTimesQueue[i][RETURNTIME]=returnTime+1;
				returnTime=0;
			}


			if (processTable[i].state==NEW || (processTable[i].state==READY && notExecutedYet[i]==0)){ //Sumamos el tiempo de respuesta 
				responseTime=processesTimesQueue[i][RESPONSETIME];
				processesTimesQueue[i][RESPONSETIME]=responseTime+1;
				responseTime=0;
			}
			else if (processTable[i].state==READY && notExecutedYet[i]==1){
				waitingTime=processesTimesQueue[i][WAITINGTIME];
				processesTimesQueue[i][WAITINGTIME]=waitingTime+1;
				waitingTime=0;
			}
		} else {
			returnTime = processesTimesQueue[executingProcessID][RETURNTIME];
...
void OperatingSystem_PrintProcessTimes(int pid){
	ComputerSystem_DebugMessage(550, SYSPROC, "response time", pid, programList[processTable[pid].programListIndex]->executableName, processesTimesQueue[pid][RESPONSETIME]);
	ComputerSystem_DebugMessage(550, SYSPROC, "return time", pid, programList[processTable[pid].programListIndex]->executableName, processesTimesQueue[pid][RETURNTIME]);
	ComputerSystem_DebugMessage(550, SYSPROC, "waiting time", pid, programList[processTable[pid].programListIndex]->executableName, processesTimesQueue[pid][WAITINGTIME]);
}